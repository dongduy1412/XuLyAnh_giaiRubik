...
def get_mean_lab(cell: np.ndarray, margin_ratio: float = 0.25) -> np.ndarray:
    sample = inner_cell_crop(cell, margin_ratio)
...
def lab_distance(color_a: np.ndarray, color_b: np.ndarray) -> float:
    diff = color_a.astype(float) - color_b.astype(float)
...
def extract_reference_colors(cells_by_face: dict[str, list[np.ndarray]]) -> dict[str, np.ndarray]:
    ...
    for face in FACE_ORDER:
        ...
        references[face] = get_mean_lab(cells[4])
    return references


def get_reference_distances(cell_lab: np.ndarray, references: dict[str, np.ndarray]) -> dict[str, float]:
    return {face: lab_distance(cell_lab, reference) for face, reference in references.items()}


def classify_cell(cell_lab: np.ndarray, references: dict[str, np.ndarray]) -> tuple[str, float, float, dict[str, float]]:
    distance_by_face = get_reference_distances(cell_lab, references)
...
def recognize_faces(
    cells_by_face: dict[str, list[np.ndarray]],
    references: dict[str, np.ndarray] | None = None,
) -> tuple[dict[str, list[str]], dict[str, list[dict[str, float | str]]]]:
    ...
    for face in FACE_ORDER:
        ...
        for index, cell in enumerate(cells_by_face[face]):
            cell_lab = get_mean_lab(cell)
            label, distance, confidence_gap, distance_by_face = classify_cell(cell_lab, references)
            labels.append(label)
            details.append({
                "index": index,
                "label": label,
...
def validate_reference_separation(references: dict[str, np.ndarray], min_distance: float = 25.0) -> list[str]:
    ...
    for i, face_a in enumerate(faces):
        for face_b in faces[i + 1:]:
            distance = lab_distance(references[face_a], references[face_b])
